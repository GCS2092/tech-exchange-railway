...
<body class="font-sans antialiased">
    ...
    @if(auth()->check())
        ...
        @if($tourId)
            <x-onboarding-tour 
                :tourId="$tourId" 
                :autoStart="App\Helpers\OnboardingHelper::shouldAutoStart($tourId, auth()->id())" 
            />
            <x-onboarding-button :tourId="$tourId" />
            <script>
                {!! App\Helpers\OnboardingHelper::generateTourScript($tourId) !!}
            </script>
        @endif
    @endif
