@props(['tourId', 'position' => 'fixed', 'class' => ''])

<button 
    onclick="startTour('{{ $tourId }}')" 
    class="onboarding-button {{ $position === 'fixed' ? 'fixed bottom-4 right-4' : '' }} z-30 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white px-4 py-2 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 {{ $class }}"
